...
var Server = function(options, app_config) {
    ...
    app.configure(function(){
        ...
        app.set('redis_channels', app_config.redis.channels);
        app.set('redis_keys', app_config.redis.keys);
        app.set('redis_host', app_config.redis.host);
        app.set('redis_port', app_config.redis.port);
        app.set('rest_client', rest_client);
        app.set('use_rest', app_config.use_rest);
        app.set('use_fixtures', app_config.use_fixtures);
        app.set('resource_scheme', app_config.resource_scheme || 'https');
        app.set('rest_backend', app_config.rest.host + (app_config.rest.port ? ':'+app_config.rest.port : ''));
        app.set('publisher_frontend', app_config.publisher.host + (app_config.publisher.port ? ':'+app_config.publisher.port : ''))
        app.use(express.favicon());
        app.use(express.logger('dev'));
        app.set('verbose_startup', app_config.verbose_startup);
        
        // WARNING For dev purposes only
        if(app_config.resource_session_id) {
            app.set('session_id', app_config.resource_session_id);
            console.log('starting with session_id', app.get('session_id'));
        }

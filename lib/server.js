...
var Server = function(options, app_config) {
	...
	app.configure(function() {
		...
		if (!detectDeepPropertyExistence(search, options)) {
			throw new Error('No Memcached configuration found at "' + search.join('.') + '"');
		}

		// If we don't get an error by now, assume we have the config object for Memcached
		var mem_config = options['Plugin::Cache'].backends.
	default;

		// Build the array of pools, deduplicate, return in order of appearance (demoting backups)
		var mem_pool = [];
		_.each(mem_config.pools, function(a, b, c) {
			if (b % 2 !== 0) {
				mem_pool = _.union(mem_pool, a);
			}
		});

		// Bail if there are no servers listed (or there is some misconfiguration)
		if (mem_pool.length === 0) {
			throw new Error('No Memcached servers found for the pool');
		}

		// Otherwise, light it up
		var mem_options = {
			hosts: mem_pool,
			prefix: mem_config.key_prefix
		};

		var cookie_parser = express.cookieParser(app_config.session_secret);
		var session_store = new MemcachedStore(mem_options);

		var SessionSockets = require('session.socket.io');
		var sessionSockets = new SessionSockets(io, session_store, cookie_parser);
		...
		app.set('session_key', 'connect.sid');
		...
		_.each(session_manager.pools, function(a, b, c) {
			if (b % 2 !== 0) {
				session_pool = _.union(session_pool, a);
			}
		});
